...
//    }
    ...
    int getNumOfClassOut(string classToCheck, CXXRecordDecl *decl){
        ...
        int allClassInMethod = 0;
        string temp;
        for (auto f = decl->field_begin() ; f != decl->field_end() ; f++ ){
            temp = ((*f)->getParent())->getNameAsString();
            if (classToCheck==temp){
                numOfClassFields++;
            }
        }
        for (auto f = decl->method_begin() ; f != decl->method_end() ; f++ ) {
            allClassInMethod += countStringInMethod(*f,classToCheck);
        }
        int numOfFunOut = allClassInMethod + numOfClassFields;
        return numOfFunOut;
    }
