...
class DispersedCouplingRule : public AbstractASTVisitorRule<DispersedCouplingRule>
{

public:
    virtual const string name() const override
    {
        return "dispersed coupling";
    }
...
.. code-block:: cpp
    ...
    bool VisitCXXRecordDecl(CXXRecordDecl *decl)
    {
        ...
        for (auto method = decl->method_begin() ; method != decl->method_end() ; method++ ){
            ...
            if (cint > DISPERSED_COUPLING_CINT &&  cdisp >= DISPERSED_COUPLING_CDISP && maxNesting > DISPERSED_COUPLING_MAXNESTING)
            {
                string description = "C++ class has dispersed coupling.";
...
static RuleSet rules(new DispersedCouplingRule());
