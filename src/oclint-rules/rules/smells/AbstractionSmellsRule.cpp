...
class AbstractionSmellsRule : public AbstractASTVisitorRule<AbstractionSmellsRule>
{
private:
    int _threshold;
    Graph graph;
...
.. code-block:: cpp
    ...
    {
            _threshold = RuleConfiguration::intForKey("ABSTRACTION_SMELLS", 1);
    ...
    bool hasMultifacetedAbstraction(CXXRecordDecl *decl){
        if (getLCOM(decl) >= MULTIFACETED_ABSTRACTION_LARGE_LCOM && (this->classMethods).size() >= MULTIFACETED_ABSTRACTION_MANY_METHODS && (this->classFields).size() >= MULTIFACETED_ABSTRACTION_MANY_FIELDS){
            return true;
        }
        return false;
    }
...
static RuleSet rules(new AbstractionSmellsRule());
