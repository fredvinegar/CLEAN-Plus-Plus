...
# install packages
RUN apt-get update && apt-get install -y apt-utils curl git gcc gdb vim perl make python3 cmake build-essential bear ninja-build bc module-assistant dkms doxygen subversion lcov libssl-dev libncurses-dev libboost-all-dev automake && rm -rf /var/lib/apt/lists/*
